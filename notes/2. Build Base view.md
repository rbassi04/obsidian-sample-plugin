...
Base is a way to display your notes as tables, cards, lists, etc. A Base has different views which is different ways to display your data.


```js
import { BasesView, HoverParent, HoverPopover, Keymap, parsePropertyId, Plugin, QueryController } from "obsidian";
...
export default class MyPlugin extends Plugin {
  async onload() {
    ...
    // and also display it as an option
    this.registerBasesView(ExampleViewType, {
      name: 'Example',
      icon: 'lucide-graduation-cap',
      factory: (controller, containerEl) => {
        // What the Base View will look like.
        // contorller => way to link plugin to Base View
        // containerEl => The container element
...
class MyBasesView extends BasesView implements HoverParent {
